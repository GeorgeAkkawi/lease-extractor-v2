...
export default function TaxSection({ propId, year, expense }) {
  ...
  const add = useMutation({
    mutationFn: async () => {
      const item = await addTaxLineItem({ property_id: propId, year, label: label.trim() || 'Property tax', amount: Number(amount) || 0, paid_date: paidDate || null });
      await carryThrough();
      return item;
    },
    onSuccess: (item) => {
  ...
  const remove = useOptimisticRemove({
    ...
    onSuccess: (_data, it) => {
      ...
      setSaved({
        ...
        undo: async () => { await addTaxLineItem({ property_id: propId, year, label: it.label, amount: it.amount, paid_date: it.paid_date || null }); await carryThrough(); },
      });
    },
  });
  ...
  // Fixing which instalment this is, or the day it cleared. Neither figure is billed, so
  // no carryThrough() — see updateExpenseLineItem.
          ...
          <div className={`cam-row${i === items.length - 1 ? ' last' : ''}`} key={it.id}>
            <div className="cam-name">
              <InlineEdit
                ...
                title="Rename this payment — which instalment it is, or which county billed it."
